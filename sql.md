...
insert into public.mdm_model_fields ( definition_id, name, code, data_type, length, precision, is_required, is_primary, is_unique, default_value, sort, status, remarks ) select v_new_id, name, code, data_type, length, precision, is_required, is_primary, is_unique, default_value, sort, status, remarks from public.mdm_model_fields where definition_id = p_definition_id order by sort asc, created_at asc;
